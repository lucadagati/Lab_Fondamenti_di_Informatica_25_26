...
nuoveRighe = table( ...
    ...
    {'inserito con sqlwrite'; 'inserito con sqlwrite'}, ... % note: evitiamo valori NULL nel fetch
    'VariableNames', {'visita_id', 'tipo_esame_id', 'valore', 'unita', 'data_risultato', 'note'} ...
    );

sqlwrite(conn, 'esami_lab', nuoveRighe);

queryVerifica = [ ...
    'SELECT e.id, e.visita_id, e.tipo_esame_id, e.valore, e.unita, e.data_risultato, ' ...
    'COALESCE(e.note, '''') AS note, t.codice, t.nome ' ...
    'FROM esami_lab e ' ...
